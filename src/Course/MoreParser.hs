...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok p s pa = between (charTok p) (charTok s) pa
...
hex ::
  Parser Char
hex =
  (\cs -> optional (pure . chr) (constantParser $ UnexpectedString cs) $ readHex cs)
...
eof ::
  Parser ()
eof = P $ \i -> if isEmpty i then Result Nil () else ExpectedEof i
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma p s pa =
  betweenCharTok p s $ sepby pa (charTok ',')
