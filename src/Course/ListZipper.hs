...
findLZ' :: (a -> Bool) -> List a -> Optional (List a, a, List a)
findLZ' p xs =
  case br of
    Nil -> Empty
    (a :. as) -> Full (reverse bl, a, as)
  where (bl, br) = break p xs
...
findLeft p (ListZipper l x r) =
  case findLZ' p l of
    Empty -> IsNotZ
    Full (r', x', l') -> IsZ $ ListZipper l' x' (r' ++ (x :. r))
...
findRight p (ListZipper l x r) =
  case findLZ' p r of
    Empty -> IsNotZ
    Full (l', x', r') -> IsZ $ ListZipper (l' ++ (x :. l)) x' r'
...
swapLeft (ListZipper (x' :. l) x r) = IsZ $ ListZipper (x :. l) x' r
...
swapRight (ListZipper l x (x' :. r)) = IsZ $ ListZipper l x' (x :. r)
...
moveLeftN n lz
  ...
  | n < 0 = moveLeftN (n + 1) -<< (moveRight lz)
  | otherwise = IsZ lz
...
moveLeftN' n lz =
  case moveLeftN n lz of
    IsZ lz' -> Right lz'
    IsNotZ -> Left $ length $ f lz
  where
    f = if n > 0 then lefts else rights
...
nth ::
  ...
  -> MaybeListZipper a
nth n lz
  | n < 0 || n >= length l = IsNotZ
  | otherwise =
    let (x :. r) = drop n l
    in IsZ $ ListZipper (reverse $ take n l) x r
  where l = toList lz
...
instance Extend ListZipper where
  ...
  (<<=) f lz = ListZipper (g moveLeft) (f lz) (g moveRight)
    where
      g h = unfoldr ((<$>) (\x -> (f x, x)) <$> toOptional . h) lz
...
instance Traversable ListZipper where
  ...
  traverse f (ListZipper l x r) = lift3 ListZipper (sequence $ f <$> l) (f x) (sequence $ f <$> r)
...
instance Traversable MaybeListZipper where
  -- travers :: (a -> f b) -> MaybeListZipper a -> f (MaybeListZipper b)
