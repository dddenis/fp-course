...
instance Functor f => Functor (StateT s f) where
  (<$>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (<$>) f (StateT sa) = StateT $ \s -> (\(a, s') -> (f a, s')) <$> sa s
...
instance Monad f => Applicative (StateT s f) where
  ...
  (<*>) ::
   StateT s f (a -> b)
    -> StateT s f a
    -> StateT s f b
  (<*>) (StateT sfab) (StateT sa) = StateT $ \s -> do
    (ab, s') <- sfab s
    (a, s'') <- sa s'
    pure (ab a, s'')
...
exec' ::
  ...
  -> s
  -> s
exec' st = snd . runExactlyOne . runStateT st
...
eval' ::
  ...
  -> s
  -> a
eval' st = fst . runExactlyOne . runStateT st
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF xs = evalT (filtering (\x -> StateT $ f x) xs) S.empty
  where
    f x s = if x > 100 then Empty else Full $ (S.notMember x s, S.insert x s)
...
instance Monad f => Applicative (OptionalT f) where
  ...
  -- (<*>) (OptionalT oab) (OptionalT oa) = OptionalT $ do
  --   ab <- oab
  --   a <- oa
  --   onFull (\a' -> pure $ (\ab' -> ab' a') <$> ab) a
...
distinctG xs =
  runOptionalT $ evalT (filtering (\x -> StateT $ \s -> OptionalT $ Logger (logEntry x) $ nextState x s) xs) S.empty
  where
    logEntry x
      | x > 100 = ("aborting > 100: " ++ show' x) :. Nil
      | even x = ("even number: " ++ show' x) :. Nil
      | otherwise = Nil
    nextState x s = if x > 100 then Empty else Full (S.notMember x s, S.insert x s)
