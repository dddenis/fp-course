...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser a
    -> Parser b
  (=<<) f pa = P $ \i -> onResult (parse pa i) (\i' a -> parse (f a) i')
...
surnameParser = lift3 (\c cs1 cs2 -> c :. cs1 ++ cs2) upper (thisMany 5 lower) (list lower)
...
phoneParser ::
  Parser Chars
phoneParser = (\phone -> is '#' *> pure phone) =<< lift2 (:.) digit phoneBodyParser
...
personParser = Person
  <$> (trim ageParser)
  <*> (trim firstNameParser)
  <*> (trim surnameParser)
  <*> (trim smokerParser)
  <*> phoneParser
  where
    trim p = lift2 const p spaces1
